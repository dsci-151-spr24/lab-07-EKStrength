...
***There are `r nrow(Whickham)` observations in this data set. Each observation represents one particular woman in the study.*** 
...
***There are `r ncol(Whickham)` variables in the dataset. Both `outcome` and `smoker` are character data that are two level factors and `age` is numerical.***

```{r}

ggplot(data=Whickham, aes(y=outcome, fill = outcome)) + 
  geom_bar() + 
  ggtitle("Distrubution of Outcome")
...
ggplot(data=Whickham, aes(y=smoker, fill = smoker)) + 
  geom_bar() + 
  ggtitle("Distrubution of Smokers")
